...
resource "azurerm_application_gateway" "app_gw" {
  ...
  gateway_ip_configuration {
    ...
    subnet_id    = "${azurerm_virtual_network.vnet.id}/subnets/${azurerm_subnet.externallb_subnet_01.name}"
  }
  ...
  backend_address_pool {
    ...
    ip_address_list = ["${azurerm_network_interface.proxy.*.private_ip_address}"]
  }
  ...
  probe {
    name                = "${azurerm_virtual_network.vnet.name}-probe"
    protocol            = "Http"
    path                = "/elb_status"
    host                = "127.0.0.1"
    interval            = 30
  ...
  ssl_certificate {
    name     = "${azurerm_virtual_network.vnet.name}-cert"
  ...
  http_listener {
    ...
    protocol                        = "Https"
    ssl_certificate_name            = "${azurerm_virtual_network.vnet.name}-cert"
  }
