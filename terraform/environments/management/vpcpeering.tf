...
resource "aws_route" "dmz_mgmt_vpc_peer_route" {
  route_table_id = "${data.terraform_remote_state.woa-belfast.dmz_vpc_id.main_route_table_id}"
...
resource "aws_route" "mgmt_dmz_vpc_peer_route" {
  ...
  destination_cidr_block = "${data.terraform_remote_state.woa-belfast.dmz_vpc_id.cidr_block}"
  vpc_peering_connection_id = "${aws_vpc_peering_connection.dmz_mgmt_vpc_peer.id}"
}
